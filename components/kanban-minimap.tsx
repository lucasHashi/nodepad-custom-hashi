...
export function KanbanMinimap({ columns, onColumnClick }: KanbanMinimapProps) {
  ...
  return (
    <div className="flex items-center gap-1.5 p-1 rounded-lg bg-popover/40 backdrop-blur-md border border-border/40 shadow-xl transition-all hover:bg-secondary/40">
      {columns.map((col) => (
        <button
          key={col.id}
          onClick={() => onColumnClick(col.id)}
          className="group relative flex items-center justify-center h-8 w-8 rounded-md hover:bg-secondary/50 transition-all active:scale-95"
          title={col.title}
        >
